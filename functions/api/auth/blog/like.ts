...
export async function onRequest(context: { request: Request; env: Env }): Promise<Response> {
  ...
  if (!viewerLogin) return jsonError('Failed to identify user', 401, request)
  ...
  try {
    ...
    if (!discussion) {
      ...
      }`, env.GITHUB_TOKEN, { repoId: REPO_ID, categoryId: CATEGORY_ID, dTitle: discussionTitle, dBody: `Comments and likes for "${title}"` }) as any
    ...
    const existingReaction = (discussion.reactions?.nodes || []).find((r: any) => r.content === 'HEART' && r.user?.login === viewerLogin)
    let liked: boolean

    if (existingReaction) {
      await ghFetch(`mutation($subjectId: ID!) { removeReaction(input: {subjectId: $subjectId, content: HEART}) { reaction { id } } }`, token, { subjectId: discussion.id })
      liked = false
    } else {
      await ghFetch(`mutation($subjectId: ID!) { addReaction(input: {subjectId: $subjectId, content: HEART}) { reaction { id } } }`, token, { subjectId: discussion.id })
      liked = true
    }

    const countData = await ghFetch(`query($nodeId: ID!) {
      node(id: $nodeId) { ... on Discussion { reactions(first: 100) { nodes { content } } } }
  ...
  } catch { return jsonError('Failed to update like', 500, request) }
}
