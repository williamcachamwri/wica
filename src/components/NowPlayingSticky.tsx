...
export function NowPlayingSticky() {
  ...
  return (
    <motion.div
      className={`nowplaying-sticky${collapsed ? ' nowplaying-sticky--collapsed' : ''}${!isNotListening ? ' nowplaying-sticky--active' : ''}`}
      initial={{ opacity: 0, y: 16, scale: 0.9 }}
      animate={{ opacity: 1, y: 0, scale: 1 }}
      transition={{ type: 'spring', stiffness: 260, damping: 22, mass: 0.8 }}
      title={!collapsed && !isNotListening ? title : undefined}
    >
      <button
        type="button"
      ...
      <div className="nowplaying-sticky__body-wrapper">
        {!isNotListening ? (
          ...
          >
            <div className="nowplaying-sticky__spotify-icon">
              <SpotifyIcon />
              <motion.span
                className="nowplaying-sticky__pulse-dot"
...
function SpotifyIcon({ muted }: { muted?: boolean }) {
  return (
    <svg
      className="nowplaying-sticky__icon"
      viewBox="0 0 24 24"
      fill="currentColor"
      xmlns="http://www.w3.org/2000/svg"
      style={muted ? { color: 'var(--text-subtle)' } : undefined}
    >
